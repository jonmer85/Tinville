...
class SendOrderMixin(object):
    ...
    def send_new_order_email(self, top_level_order, request):
        ...
        for order in orders:
            ...
            messages = CommunicationEventType.objects.get_and_render(
                code, self.get_message_context(user, order, request))

            if messages and messages['body']:
                dispatcher = Dispatcher(logger)
                dispatcher.dispatch_user_messages(user, messages)

    def get_message_context(self, user, order, request):
        ctx = {
            'user': user,
            'order': order,
            'lines': order.lines.all(),
            'url': request.build_absolute_uri(reverse('dashboard:order-detail',

...
from custom_oscar.apps.order.tasks import pay_designers, pay_promoters
from common.factories import create_order, create_product, create_basket_with_products, create_basket
from user.models import TinvilleUser, DesignerPayout, PromoterPayout
...
class PayoutTests(TestCase):
    ...
    def setUp(self):
        ...
        self.assertEqual(len(PromoterPayout.objects.all()), 0, "No promoter payout should be recorded yet")
    ...
    def test_no_designer_payout_if_shipped_but_not_in_transit(self):
        self.order = create_order(number="2-10001", user=self.user, shop=self.shop)
    ...
    def assert_proper_designer_payment_events(self, total_payment_events, payment_event_group, payout_total):
    ...
    def assert_proper_promoter_payment_events(self, total_payment_events, payment_event_group, payout_total):
        self.assertEqual(len(PaymentEvent.objects.all()), total_payment_events,
                         "Another payment event should exist showing that the 1 shipping event was paid out")
        # There should be 1 payment event since there was only 1 shipped package to payout
        promoter_payment_event = PaymentEvent.objects.get(
            Q(event_type=PaymentEventType.objects.get(code="paid_promoter")) &
            Q(group=payment_event_group))
        self.assertEqual(
            promoter_payment_event.group, payment_event_group,
            "Promoter payment event should be of the same group as the shipping paid event")
        self.assertAlmostEqual(
            promoter_payment_event.amount,
            Decimal(payout_total),
            places=2,
            msg="The amount paid should be the item amount times 0.03. expected: " + str(promoter_payment_event.amount) + " actual: " + str(payout_total))
        return promoter_payment_event

    def assert_proper_designer_payout_records(self, total_payout_records, payment_event_ref, payout_total):
    ...
    def assert_proper_promoter_payout_records(self, total_payout_records, payment_event_ref, payout_total):
        promoter_payout = PromoterPayout.objects.get(id=payment_event_ref)
        self.assertEqual(len(PromoterPayout.objects.all()), total_payout_records)
        self.assertAlmostEqual(
            promoter_payout.amount,
            Decimal(payout_total),
            places=2)
        return promoter_payout
    ...
    def test_designer_payout_on_one_full_order_bank_account(self):
        self.order = create_order(number="2-10001", user=self.user, shop=self.shop)
        shipped_event, in_transit_event = self.create_basic_shipping_and_payment_events()
        rp = stripe.Recipient.retrieve(self.user.recipient_id)
        rp.bank_account = \
    ...
    def test_designer_no_payout_if_shipping_not_paid(self):
        self.order = create_order(number="2-10001", user=self.user, shop=self.shop)
    ...
    def test_designer_no_payout_if_payment_already_made(self):
        self.test_designer_payout_on_one_full_order()
    ...
    def test_designer_no_payout_due_to_no_stripe_recipient_id_for_designer(self):
    ...
    def test_designer_payout_of_partial_order_full_line_item(self):
        ...
        designer_payment_event2 = self.assert_proper_designer_payment_events(
            total_payment_events=4, payment_event_group=in_transit_event2.group, payout_total=24.00)

        designer_payout2 = self.assert_proper_designer_payout_records(
    ...
    def test_designer_payout_of_partial_order_partial_quantity_line_item(self):
    ...
    def test_designer_payout_of_multiple_orders(self):
        self.order = create_order(number="2-10001", user=self.user, shop=self.shop)
    ...
    def test_designer_multiple_payouts_if_orders_between_pay_periods(self):
        self.test_designer_payout_on_one_full_order()
        self.assertEqual(len(PaymentEvent.objects.all()), 2)
        self.assertEqual(len(DesignerPayout.objects.all()), 1, "1 payout should exist for first payout")
    ...
    def test_promoter_payout_on_one_full_order(self):
        ...
        promoter_payment_event = self.assert_proper_promoter_payment_events(
            total_payment_events=2, payment_event_group=in_transit_event.group, payout_total=0.29)
    ...
    def test_promoter_no_payout_due_to_no_stripe_recipient_id_for_promoter(self):
        ...
        # Make sure the exception rolled back any payment and payout events caused
        with self.assertRaises(PaymentEvent.DoesNotExist):
            PaymentEvent.objects.get(event_type=PaymentEventType.objects.get(code="paid_promoter"))
    ...
    def test_promoter_payout_of_partial_order_full_line_item(self):
        ...
        self.order = create_order(number="2-10001", basket=create_basket_with_products(products),
                                  user=self.user, shop=self.shop, promoter=self.user)

        # promoter only shipped first item
        first_item = self.order.lines.get(product=products[0])
        shipped_event, in_transit_event = \
            self.create_basic_shipping_and_payment_events(
                shipping_price=5.00, lines=[first_item], line_quantities=[first_item.quantity])
        ...
        promoter_payment_event = self.assert_proper_promoter_payment_events(
            total_payment_events=2, payment_event_group=in_transit_event.group, payout_total=0.59)

        promoter_payout = self.assert_proper_promoter_payout_records(
            total_payout_records=1, payment_event_ref=promoter_payment_event.reference, payout_total=0.59)

        self.assert_proper_stripe_records(promoter_payout)

        # Now ship the next item and pay the promoters again
        second_item = self.order.lines.get(product=products[1])
        shipped_event2, in_transit_event2 = \
            self.create_basic_shipping_and_payment_events(
                shipping_price=10.00, lines=[second_item], line_quantities=[second_item.quantity])
        ...
        promoter_payment_event2 = self.assert_proper_promoter_payment_events(
            total_payment_events=4, payment_event_group=in_transit_event2.group, payout_total=1.19)

        promoter_payout2 = self.assert_proper_promoter_payout_records(
            total_payout_records=2, payment_event_ref=promoter_payment_event2.reference, payout_total=1.19)
    ...
    def test_promoter_payout_of_partial_order_partial_quantity_line_item(self):
        ...
        # promoter only shipped partial quantities of both items
        shipped_event, in_transit_event = \
            self.create_basic_shipping_and_payment_events(
                shipping_price=5.00, lines=self.order.lines.all(), line_quantities=[2, 3])
        ...
        promoter_payment_event = self.assert_proper_promoter_payment_events(
            total_payment_events=2, payment_event_group=in_transit_event.group, payout_total=4.79)

        promoter_payout = self.assert_proper_promoter_payout_records(
            total_payout_records=1, payment_event_ref=promoter_payment_event.reference, payout_total=4.79)
        ...
        promoter_payment_event2 = self.assert_proper_promoter_payment_events(
            total_payment_events=4, payment_event_group=in_transit_event2.group, payout_total=1.79)

        promoter_payout2 = self.assert_proper_promoter_payout_records(
            total_payout_records=2, payment_event_ref=promoter_payment_event2.reference, payout_total=1.79)
    ...
    def test_promoter_multiple_payouts_if_orders_between_pay_periods(self):
        ...
        promoter_payment_event = self.assert_proper_promoter_payment_events(
            total_payment_events=4, payment_event_group=in_transit_event.group, payout_total=0.29)

...
function RemoveCartItem(itemId)
{
    ...
    function updateItemView(changeGender){
        ...
        $("#shopItems").load(" #shopItems", $.param({genderfilter: genderFilter, typefilter: typeFilter, sortfilter: sortFilter}, function(){$(this).children().unwrap()}));
        if (genderFilter.match("All"))
            genderFilter = "All";
        if (changeGender)
            refreshtypes(genderFilter, typeFilter);
    }
    var myresponse;
    function refreshtypes(genderFilter, typeFilter) {
        if (typeFilter.match("All")) {
